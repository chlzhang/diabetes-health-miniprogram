...
function buildComprehensivePlan(localFull, ctx) {
  ...
  return {
    ...
    warnings: risk.level === 'high' ? ['风险等级偏高，建议尽快就医复查'] : [],
    nextWeekGoals: [
      '连续 5 天完成今日运动目标',
      '晚餐主食减半，提前到 19 点前完成',
      '每天餐后步行 10-15 分钟有助控糖'
...
function buildChatFallback(ctx, userMessage) {
  ...
  var msg = userMessage || '';
  var reply = '';
  if (/血糖|空腹|餐后|tir|糖化/.test(msg)) {
    reply = 'AI 助手暂时不可用，已切换到本地建议。您的整体健康评分 ' + score + '，血糖达标率 TIR ' + tir + '%，风险等级 ' + risk.label + '。建议优先关注「餐后 2 小时」值，目标 ≤ 7.8 mmol/L；如频繁超标请就医。';
    ...
    reply = 'AI 助手暂时不可用。睡眠和压力对血糖影响很大，建议保持规律作息（7-8 小时），尽量在 23 点前入睡。';
  } else {
